...
module.exports = function DWGuide(mod) {
    ...
    function spawnLoc(degrees, radius) {
        ...
        const finalrad = bossLoc.w - rads;

        const spawnx = bossLoc.x + radius * Math.cos(finalrad);
        const spawny = bossLoc.y + radius * Math.sin(finalrad);
    ...
    function darkanInOutRange() {
        for (let degree = 0; degree < 360; degree += 360 / 20) {
            spawnFlower(spawnLoc(degree, 300), 6000, collectionIds[0]);
        }
    }
